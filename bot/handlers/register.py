...
async def register_start(msg: Message, state: FSMContext):
    ...
    if existing and existing['is_approved']:
        await msg.answer(
            f"✅ Ваш бизнес *{existing['name']}* уже активен в каталоге!\n\n"
    ...
    await msg.answer(
        ...
        "Размещение бесплатное. После проверки вы появитесь в каталоге.\n\n"
        "1️⃣ из 6️⃣ — *Выберите категорию:*",
        parse_mode="Markdown",
...
async def reg_district(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        f"Район: *{dist['name']}* ✅\n\n"
        "3️⃣ из 6️⃣ — Введите *название* вашего бизнеса или своё имя:\n\n"
...
async def reg_name(msg: Message, state: FSMContext):
    if len(msg.text.strip()) < 2:
        await msg.answer("⚠️ Название слишком короткое. Попробуйте ещё раз:"); return
    await state.update_data(name=msg.text.strip())
    await msg.answer(
        "4️⃣ из 6️⃣ — Введите *номер телефона:*\n\n_Например: +996 700 123 456_",
        parse_mode="Markdown"
    )
...
async def reg_phone(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "5️⃣ из 6️⃣ — Опишите ваш бизнес или услуги:\n\n"
        "_Например: Уютное кафе, завтраки и обеды. Домашняя кухня. Есть доставка._",
        parse_mode="Markdown"
    )
...
async def reg_desc(msg: Message, state: FSMContext):
    if len(msg.text.strip()) < 10:
        await msg.answer("⚠️ Описание слишком короткое, расскажите подробнее:"); return
    await state.update_data(description=msg.text.strip())
    await msg.answer(
        "6️⃣ из 6️⃣ — Введите *адрес*:\n\n"
        "_Например: ул. Токтогула 12, рядом с рынком_\n"
        "_Или просто: Центр Каракола_\n"
...
async def reg_address(msg: Message, state: FSMContext):
    ...
    await msg.answer(summary, parse_mode="Markdown", reply_markup=confirm_keyboard())
...
async def reg_confirm(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        ...
        "Наш модератор проверит данные и активирует ваш профиль в течение 24 часов.\n"
        "Вы получите уведомление здесь в боте.",
        parse_mode="Markdown"
    )
